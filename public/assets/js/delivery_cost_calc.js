var ground_rate = 0;
var ocean_rate = 0;
$('select[name="auction"]').change(function(){
    var value = $(this).val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.total_price=="empty")
                {
                    ...
                    $('select[name="country"]').val($("select[name='country'] option:first").val());
                    $('select[name="country"]').trigger('refresh');
                    $('.costdel-calc__total-side:eq(0) .amount').text('');
                    $('.costdel-calc__total-side:eq(1) .amount').text('');
                    $('.costdel-calc__total-side:eq(2) .amount').text('');
                }else
                {
                    $('.costdel-calc__total-side:eq(0) .amount').text("$"+data.ground_rate);
                    $('.costdel-calc__total-side:eq(1) .amount').text("$"+data.ocean_price);
                    $('.costdel-calc__total-side:eq(2) .amount').text("$"+data.total_price);
                }
    ...
    }else
    {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // $('.price_span').text(data.ground_rate);
...
$('input[name="vehicle_type"]').on('change', function(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.costdel-calc__total-side:eq(1) .amount').text("$"+data.ocean_price);
            $('.costdel-calc__total-side:eq(2) .amount').text("$"+data.total_price);
            $('.costdel-calc__content .paragraph--cost').text(data.comment);
