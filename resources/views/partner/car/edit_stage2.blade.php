            ...
            <div class="container">
                ...
                <div class="user-block">
                    <div class="user-block__content">
                        <div class="user-block__back"><a class="arrow-link arrow-link--reverse" href="{{route('partner_car.create.update', $model->id)}}">{{App\Models\Translation::getTranslWord($words, $sel_lang, "back")}}</a></div>
                        <div class="user-block__nav">
                            <ul>
                                <li><a href="{{route('partner_car.edit', $model->id)}}">
                        ...
                        <div class="user-block__add-car-add-media">
                            <p>{{App\Models\Translation::getTranslWord($words, $sel_lang, "before_adding_photos")}} <a class="underline" href="{{route('media_requirements.page')}}?route=partner_car.create.stage2&id={{$model->id}}">{{App\Models\Translation::getTranslWord($words, $sel_lang, "requirements_posted_photos")}}</a></p>
                            <!-- <div class="user-block__add-car-media"> -->
                            <div class="user-block__add-car-media-text">{{App\Models\Translation::getTranslWord($words, $sel_lang, "first_photo_text")}}</div>
                                ...
                                @foreach($model->images->sortBy('order_id') as $image)
                                    <div class="image-card">
                                        ...
                                        <div class="image-card__controlls">
                                            @if($loop->iteration!=1)
                                                <a class="image-card__btn image-card__btn--top" href="{{route('partner_car.image.move_to_top', ["order_id"=>$loop->iteration, "id"=>$model->id])}}" title="{{App\Models\Translation::getTranslWord($words, $sel_lang, "move_up")}}"></a>
                                                <a class="image-card__btn image-card__btn--up" href="{{route('partner_car.image.move_upper', ["order_id"=>$loop->iteration, "id"=>$model->id])}}" title="{{App\Models\Translation::getTranslWord($words, $sel_lang, "move_higher")}}"></a>
                                            @endif
                                            @if($loop->iteration!=count($model->images))
                                                <a class="image-card__btn image-card__btn--down" href="{{route('partner_car.image.move_lower', ["order_id"=>$loop->iteration, "id"=>$model->id])}}" title="{{App\Models\Translation::getTranslWord($words, $sel_lang, "move_below")}}"></a>
                                            @endif
                                            <a class="image-card__btn image-card__btn--rotate" href="{{route('partner_car.image.rotate', $image->id)}}" title="{{App\Models\Translation::getTranslWord($words, $sel_lang, "to_turn")}}"></a>
                                            <a class="image-card__btn image-card__btn--delete" href="{{route('partner_car.image.delete', $image->id)}}" title="{{App\Models\Translation::getTranslWord($words, $sel_lang, "delete_photo")}}"></a>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            <div class="user-image-uploader mb-40 md:mb-40">
                                <form method="post" action="{{route('partner_car.upload-photo', $model->id)}}" enctype="multipart/form-data">
                                    <input required="required" id="user-add-new-files" type="file" name="images[]" accept="image/*" multiple data-uploaded-text="{{App\Models\Translation::getTranslWord($words, $sel_lang, "files_uploaded")}}:">
                                    @csrf
                          ...
                          </svg><span>{{App\Models\Translation::getTranslWord($words, $sel_lang, "add_images")}}</span></span></label>
                                    <input type="submit" class="sm:min-w-0 sm:w-full btn-outlined btn-outlined--theme-accent2 min-w-330 mt-35" value="{{App\Models\Translation::getTranslWord($words, $sel_lang, "upload_photo")}}">
                                </form>
                            </div>
                            {!! Form::model($model,['url'=>route('partner_car.update.stage2', $model->id),'method'=>'POST', 'files' => true]) !!}
                            <p>{{App\Models\Translation::getTranslWord($words, $sel_lang, "video_link")}}</p>
                            <input type="hidden" value="{{$model->id}}" name="id">
                            <div class="input-group mb-70 md:mb-40">
                                <input type="text" name="video" value="{{$model->video}}">
                            </div>
                            <div class="user-inputs-btn user-inputs-btn--centered">
                                <button type="submit" class="sm:min-w-0 sm:w-full btn-outlined btn-outlined--theme-accent2 min-w-330">{{App\Models\Translation::getTranslWord($words, $sel_lang, "preview")}}</button>
                            </div>
                            {!! Form::close() !!}
                        </div>
                    </div>
